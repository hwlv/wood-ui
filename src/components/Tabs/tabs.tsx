...
export const Tabs: FC<TabsProps> = (props) => {
  ...
  const navClass = classNames('wd-tabs-nav', {
    'nav-line': type === 'line',
    'nav-card': type === 'card',
  })
  const renderNavLinks = () => {
    return React.Children.map(children, (child, index) => {
      ...
      const classes = classNames('wd-tabs-nav-item', {
  ...
  return (
    <div className={`wd-tabs ${className}`}>
      <ul className={navClass}>
        {renderNavLinks()}
      </ul>
      <div className="wd-tabs-content">
        {renderContent()}
      </div>
    </div>
  )
