...
export const Select:FC<SelectProps> = (props) => {
  ...
  const containerClass = classNames('wd-select', {
  ...
  return (
    <div className={containerClass} ref={containerRef}>
      <div className="wd-select-input" onClick={handleClick}>
        <Input
          ref={input}
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition
            ...
            timeout={300}
          >
          <ul className="wd-select-dropdown">
            {generateOptions()}
          </ul>
        </Transition>
      </SelectContext.Provider>
      {multiple &&
        <div className="wd-selected-tags" style={{maxWidth: containerWidth.current - 32}}> 
          {
            selectedValues.map((value, index) => {
              return (
                <span className="wd-tag" key={`tag-${index}`}>
                  {value}
